...
class _GoatFormScreenState extends State<GoatFormScreen> {
          ...
          children: [
            Text(
              baseSource == 'Purchased' 
                ? 'Where/who did you purchase this goat from?'
                : 'Please provide additional details about how you acquired this goat (e.g., gift, inheritance, trade).',
  ...
  /// Populates form fields if editing an existing goat
  void _populateFormFields() {
    ...
    if (c != null) {
      ...
      // If we have a pre-selected classification for new goat
  ...
  void _handleClassificationSelection(String classification) {
    setState(() {
      ...
      // Auto-fill sex based on classification (only for new goat with pre-selected classification)
      if (widget.preSelectedClassification != null && widget.goat == null) {
        if (classification == 'Doe' || classification == 'Doeling') {
          _sex = 'Female';
        } else if (classification == 'Buck' || classification == 'Buckling') {
          _sex = 'Male';
        }
  ...
  Future<void> _submitForm() async {
    ...
    // Check if tag already exists (only for new goat or when tag is changed)
    bool shouldCheckTag = !_isEditing || widget.goat!.tagNo != tagToCheck;
      ...
      try {
        ...
        if (duplicateExists) {
          messenger.showSnackBar(
            const SnackBar(content: Text('Goat Tag already exists. Please use a different unique tag.')),
          );
          return;
        }
      } catch (e) {
    ...
    try {
      ...
      String errorMessage = 'Failed to save goat information';

      if (e.toString().contains('Duplicate entry') ||
          e.toString().contains('tag_no') ||
          e.toString().contains('already exists') ||
          e.toString().contains('1062')) {
        errorMessage = 'Goat Tag already exists. Please use a different unique tag.';
      } else if (e.toString().contains('foreign key constraint') ||
          e.toString().contains('Cannot delete or update a parent row')) {
        errorMessage = 'Unable to update. This goat has offspring that reference it.';
    ...
    if (success) {
      if (widget.goat == null) {
        // Show "Add Another" dialog for new goat
        _showAddAnotherDialog();
      } else {
        // For updates, just show success message and close
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('goat updated successfully!'),
            backgroundColor: AppColors.vibrantGreen,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
          ),
      ...
      _showErrorSnackBar('Failed to save goat information. Please try again.');
    }
  }

  /// Show dialog asking if user wants to add another goat of the same classification
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Text(
                'Add another ${_classification?.toLowerCase() ?? 'goat'}?',
          ...
          actions: [
            TextButton(
              onPressed: () {
                ...
                Navigator.pop(context, true); // Close form and return to goat list
              },
            ...
            TextButton(
              onPressed: () {
                Navigator.pop(context); // Close dialog
                _resetFormForNewgoat(); // Reset form for new goat of same classification
              },
              style: TextButton.styleFrom(
  ...
  /// Reset form for adding another goat of the same classification
  void _resetFormForNewgoat() {
    setState(() {
  ...
  Widget _buildSearchableDropdown({
    ...
    return DropdownButtonFormField<String>(
      ...
      items: [
        ...
        // Add all goat options, ensuring no duplicates
        ...options.toSet().map((goat) {
  ...
  Widget _buildClassificationField() {
    ...
    final isEditing = widget.goat != null;

    // Lock classification for pre-selected classifications (new goat from FAB menu)
    // For editing existing goat, allow classification changes
