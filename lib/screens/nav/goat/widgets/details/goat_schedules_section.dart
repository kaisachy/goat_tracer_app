...
class _GoatSchedulesSectionState extends State<GoatSchedulesSection> {
  ...
  Future<void> _loadSchedules() async {
    if (widget.goat.tagNo.isEmpty) {
      if (mounted) {
        setState(() {
          ...
          schedules = [];
          scheduleError = 'Goat Tag is required to load schedules';
        });
      }
      return;
    }
    ...
    try {
      ...
      log('Loading schedules for Goat Tag: "$normalizedTag"');
      final fetchedSchedules =
      await ScheduleService.getSchedulesForgoat(normalizedTag);
      ...
      log('Error loading schedules for goat ${widget.goat.tagNo}: $e');
      if (mounted) {
        setState(() {
          schedules = [];
          isLoadingSchedules = false;
    ...
    if (schedules == null || schedules!.isEmpty) {
      return Column(
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            widget.goat.tagNo.isEmpty
                ? 'Goat Tag is required to load schedules'
                : 'No schedules assigned to ${widget.goat.tagNo}',
            style: TextStyle(
              color: AppColors.textSecondary.withValues(alpha: 0.8),
              fontSize: 14,
