...
class GoatScreen extends StatefulWidget {
  ...
  @override
  State<GoatScreen> createState() => GoatScreenState();
}

// ✨ MODIFIED: Added the WidgetsBindingObserver mixin and made state public
class GoatScreenState extends State<GoatScreen>
  ...
  final GlobalKey _addGoatGridKey = GlobalKey();
  final GlobalKey _searchFilterKey = GlobalKey();
  final GlobalKey _reportTypeKey = GlobalKey();
  final GlobalKey _exportButtonsKey = GlobalKey();
  final GlobalKey _goatListKey = GlobalKey();
  final GlobalKey _goatCardOptionsKey = GlobalKey();
  ...
  Widget _buildAddgoatGrid() {
    return Showcase(
      ...
      child: _buildAddGoatGridContent(),
    );
  }

  Widget _buildAddGoatGridContent() {
    final List<Map<String, dynamic>> items = [
      ...
      },
      {
        ...
        'sex': 'Male',
        'iconPath': 'assets/images/goat-icons/weanling.png',
        'color': Colors.purple,
  ...
  void startUserGuide() async {
    ...
    await UserGuideService.resetGuide('goat');
  ...
  Widget _buildGoatOptionsMenu(Goat goat, {bool isFirstCard = false}) {
    ...
    // Wrap the first card's menu button with showcase
    if (isFirstCard) {
      return Showcase(
        key: _goatCardOptionsKey,
        title: 'Goat Options Menu',
        description: 'Access various actions for each goat: Edit details, add history records, change stage/status, export reports (Excel/PDF), archive, or delete. Tap the three-dot menu on any goat card.',
        targetShapeBorder: RoundedRectangleBorder(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        _buildGoatOptionsMenu(goat, isFirstCard: index == 0),
                      ],
                    ),
  ...
  Widget build(BuildContext context) {
    return ShowCaseWidget(
      ...
      onFinish: () async {
        ...
        await UserGuideService.markGuideCompleted('goat');
