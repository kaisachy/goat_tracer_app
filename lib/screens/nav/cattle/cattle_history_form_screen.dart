// lib/screens/nav/cattle/cattle_history_form_screen.dart

import 'package:cattle_tracer_app/screens/nav/cattle/widgets/history/history_specific_fields.dart';
import 'package:cattle_tracer_app/screens/nav/cattle/widgets/history/history_type_dropdown.dart';
...
import 'widgets/history/history_cattle_info_card.dart';
...
class CattleHistoryFormScreen extends StatefulWidget {
  final CattleHistoryRecord? historyRecord;
  final String? cattleTag;
  final String? initialHistoryType;

  const CattleHistoryFormScreen({
    super.key,
    this.historyRecord,
    this.cattleTag,
  ...
  @override
  State<CattleHistoryFormScreen> createState() => _CattleHistoryFormScreenState();
}

class _CattleHistoryFormScreenState extends State<CattleHistoryFormScreen>
  ...
  void initState() {
    ...
    // Load existing calf data if editing birth history record
  ...
  Future<Map<String, dynamic>?> _getLatestHistoryRecord({
    ...
    try {
      final all = await CattleHistoryService.getCattleHistory();
      final filtered = all
          .where((e) =>
  ...
  Future<void> _handleHistoryTypeChanged(String value) async {
    ...
    if (_cattleDetails == null) {
      debugPrint('DEBUG: _cattleDetails is null, returning early');
      return;
    }
    final cattleTag = _cattleDetails!.tagNo;
    debugPrint('DEBUG: Processing history type change for cattle: $cattleTag');
    ...
    if (value.toLowerCase() == 'pregnant') {
      debugPrint('DEBUG: Processing Pregnant history record for cattle: $cattleTag');
      final latestBreeding = await _getLatestHistoryRecord(cattleTag: cattleTag, historyType: 'Breeding');
      ...
      debugPrint('DEBUG: Found bull_tag: $bullTag, semen_used: $semen');
      ...
      if (semen != null && semen.isNotEmpty) {
        ...
        debugPrint('DEBUG: Extracted bull tag from semen: $finalBullTag');
        if (finalBullTag.isNotEmpty) {
          _controllers['bull_tag']?.text = finalBullTag;
          debugPrint('DEBUG: Set bull_tag controller to: $finalBullTag');
        }
      } else if (bullTag != null && bullTag.isNotEmpty) {
        debugPrint('DEBUG: Using direct bull_tag: $bullTag');
        _controllers['bull_tag']?.text = bullTag;
        _controllers['semen_used']?.text = '';
        debugPrint('DEBUG: Set bull_tag controller to: $bullTag');
      }
    ...
    if (value.toLowerCase() == 'gives birth') {
      debugPrint('DEBUG: Processing Gives Birth history record for cattle: $cattleTag');
      final latestPregnant = await _getLatestHistoryRecord(cattleTag: cattleTag, historyType: 'Pregnant');
      ...
      debugPrint('DEBUG: Found in pregnant history record - bull_tag: $bull, semen_used: $semen');
      if ((semen == null || semen.isEmpty) && (bull == null || bull.isEmpty)) {
        ...
        debugPrint('DEBUG: Fallback values - bull_tag: $bull, semen_used: $semen');
      }
      ...
      if (semen != null && semen.isNotEmpty) {
        ...
        debugPrint('DEBUG: Extracted bull tag from semen for Gives Birth: $bullTag');
        ...
        debugPrint('DEBUG: Using direct bull_tag for Gives Birth: $bull');
        _controllers['bull_tag']?.text = bull;
        _controllers['semen_used']?.text = '';
        debugPrint('DEBUG: Set bull_tag controller for Gives Birth to: $bull');
      }
  ...
  Future<void> _loadExistingCalfData() async {
    if (widget.historyRecord == null || widget.historyRecord!.calfTag == null) return;

    try {
      final calfTagString = widget.historyRecord!.calfTag!;
      debugPrint('Loading calf data for tag string: $calfTagString');
      ...
      if (calfTagString.contains(',')) {
        ...
        debugPrint('Found multiple calf tags: $calfTags');
        ...
        if (calfTags.isNotEmpty) {
          ...
          if (calf != null && mounted) {
            ...
            debugPrint('Loaded first calf data: ${calf.tagNo} with ID: ${calf.id}');
          }
        }
      } else {
        // Single calf
        ...
        if (calf != null && mounted) {
          ...
          debugPrint('Loaded existing calf data: ${calf.tagNo} with ID: ${calf.id}');
        }
      }
    } catch (e) {
      debugPrint('Error loading calf data: $e');
    ...
    try {
      ...
      if (breedingType != null && breedingType.isNotEmpty) {
        ...
        if (semenUsedVal.isNotEmpty) {
          ...
          debugPrint('DEBUG: Inferred breeding_type as natural_breeding from bull_tag');
        }
      }
      ...
      if (semenUsed != null && semenUsed.isNotEmpty) {
        ...
        if (currentBreedingType == 'artificial_insemination' && bullTagVal.isNotEmpty) {
          ...
          debugPrint('DEBUG: Inferred semen_used from bull_tag for AI: $bullTagVal');
        }
      }
      
      // Set bull tag
      final bullTag = eventJson['bull_tag']?.toString();
      debugPrint('DEBUG: Found bull_tag in history record: $bullTag');
      if (bullTag != null && bullTag.isNotEmpty) {
        _controllers['bull_tag']?.text = bullTag;
        debugPrint('DEBUG: Set bull_tag controller to: $bullTag');
      } else {
        debugPrint('DEBUG: No bull_tag found in history record data');
      }
      ...
      // Bull-side reciprocal hydration: if subject is male and fields are missing, try to fetch partner cow/heifer history record on same date
      try {
      ...
      // Values read below directly from controllers when needed; avoid unused locals
        ...
        if (isMaleSubject && isBreeding && eventDate.isNotEmpty) {
          ...
          if (partnerTag != null && partnerTag.isNotEmpty) {
            ...
            final allPartnerEvents = await CattleHistoryService.getCattleHistoryByTag(partnerTag);
            ...
            if (partner.isNotEmpty) {
              ...
              if (btNow == 'natural_breeding') {
                ...
                if (bullCtrl != null && bullCtrl.text.trim().isEmpty) {
                  ...
                  if (bullTagSelf.isNotEmpty) {
                    ...
                    debugPrint('DEBUG: Hydration - set bull_tag to self for natural breeding UI: $bullTagSelf');
                  }
                }
              }
            } else {
              ...
              if (bullCtrl != null && bullCtrl.text.trim().isEmpty) {
                ...
                if (bullTagSelf.isNotEmpty) {
                  bullCtrl.text = bullTagSelf;
                  debugPrint('DEBUG: Hydration Fallback - set breeding_type to natural_breeding and bull_tag to self: $bullTagSelf');
                }
              }
            }
          } else {
        ...
        debugPrint('DEBUG: Error hydrating bull reciprocal fields from partner: $e');
      }
  ...
  Future<void> _loadCattleDetails() async {
    ...
    if (cattleTag == null && isEditing && widget.historyRecord != null) {
      cattleTag = widget.historyRecord!.cattleTag;
    }
    ...
    try {
      ...
      if (cattle != null && mounted) {
        ...
        if (isEditing && widget.historyRecord != null && _cattleDetails != null) {
  ...
  Future<void> _setEditEventType(Cattle cattle) async {
    ...
    if (eventTypes.contains(editEventType)) {
      ...
      if (mounted) {
        _showWarningMessage(
            'History type "$editEventType" is not valid for ${cattle.sex.toLowerCase()} cattle. Please select a valid history type.');
      }
    }
  }
  ...
  Future<bool> _executeCalfOperations() async {
    if (_temporaryCalfData == null || _temporaryCalfData!['fullCalfData'] == null) {
      debugPrint('No calf operations to execute');
    ...
    try {
      ...
      debugPrint('Executing calf $pendingOperation for: $calfTag');
      ...
      if (pendingOperation == 'update') {
        ...
        if (calfId == null) {
          debugPrint('No calf ID found for update, trying to find by tag');
        ...
        debugPrint('Calf update result: $success for ID: $calfId');
      } else {
        ...
        debugPrint('Calf registration result: $success');
      }
      ...
      debugPrint('Error executing calf operations: $e');
      return false;
    }
  }
  ...
  Future<void> _handleGivesBirthEvent() async {
    try {
      ...
      // Prefer multi-calf from HistorySpecificFields if available
      ...
      debugPrint('Birth event results - Calf: $calfStatus, Mother: $motherStatus');
  ...
  Future<void> _handleCastratedEvent() async {
    try {
      ...
      debugPrint('Castrated event result: $cattleStatus');
  ...
  Future<void> _openCalfRegistrationDialog() async {
    ...
    if (_temporaryCalfData != null) {
      try {
        ...
        debugPrint('Prepared calf data for dialog: calfId=$calfId, isEditMode=${existingCalfData['isEditMode']}');
      } catch (e) {
        debugPrint('Error preparing calf data for dialog: $e');
    ...
    if (result != null && mounted) {
      ...
      debugPrint('Calf dialog result: ${result['pendingOperation']} operation prepared for ${result['tag_no']}');
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // Handle calf_tag specially for Gives Birth history with multiple calves
      ...
      if (selectedHistoryType.toLowerCase() == 'gives birth') {
        ...
        if (multiCalves != null && multiCalves.isNotEmpty) {
          ...
          if (calfTags.isNotEmpty) {
            ...
            debugPrint('DEBUG: Multiple calf tags collected: $calfTagValue');
          }
        }
      }
      ...
      if (selectedHistoryType.toLowerCase() == 'gives birth') {
        ...
        if (!hasMulti && !hasSingle) {
          _showErrorMessage('Please add at least one calf for birth history.');
          setState(() => _isLoading = false);
          return;
        }
      }
      ...
      debugPrint('Cattle details loaded: ${_cattleDetails != null}');
      if (_temporaryCalfData != null) {
        debugPrint('Temporary calf data: $_temporaryCalfData');
      }
      ...
      try {
        if (isEditing) {
          ...
          if (isBullSubject && isBreedingEdit && _partnerTagForEdit != null && _partnerHistoryIdForEdit != null) {
            ...
            final partnerOk = await CattleHistoryService.updateCattleHistory(partnerData);
          ...
          if (isFemaleSubject && isBreedingEdit) {
            ...
            if (partnerBullTag != null && partnerBullTag.isNotEmpty) {
              // Fetch partner bull history and find matching breeding by date
              final allPartnerEvents = await CattleHistoryService.getCattleHistoryByTag(partnerBullTag);
              ...
              if (partner.isNotEmpty) {
                ...
                // Execute bull update
                final bullOk = await CattleHistoryService.updateCattleHistory(bullData);
                debugPrint('Unified edit: bull reciprocal update result: $bullOk');
              }
            }
          }

          debugPrint('Updating event with ID: ${widget.historyRecord!.id}');
          eventSuccess = await CattleHistoryService.updateCattleHistory(data);
        } else {
          debugPrint('Creating new history record');
          eventSuccess = await CattleHistoryService.storeCattleHistory(data);
        }
  ...
  Future<void> _handleSoldEvent() async {
    try {
      ...
      if (_cattleDetails != null) {
        final cattleUpdateData = Map<String, dynamic>.from(_cattleDetails!.toJson());
        cattleUpdateData['status'] = 'Sold';
        cattleStatusUpdated = await CattleService.updateCattleInformation(cattleUpdateData);
        cattleStatus = cattleStatusUpdated
            ? 'Cattle ${_cattleDetails!.tagNo} status updated to Sold'
            : 'Failed to update cattle ${_cattleDetails!.tagNo} status to Sold';
        ...
        if (cattleStatusUpdated) {
          ...
          debugPrint('Auto-archive result for sold cattle: $cattleArchived');
        }
      }

      // Log the result for debugging
      debugPrint('Sold event result: $cattleStatus');

      // Show success feedback to user
      if (mounted && cattleStatusUpdated) {
        final message = cattleArchived 
            ? 'Cattle status updated to Sold and moved to archive'
            : 'Cattle status updated to Sold';
        
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(message),
            backgroundColor: cattleArchived ? Colors.green[600] : Colors.blue[600],
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
            duration: const Duration(seconds: 3),
          ),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Warning: Failed to update cattle status to Sold'),
            backgroundColor: Colors.orange[600],
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
            duration: const Duration(seconds: 3),
    ...
    try {
      ...
      if (returnToHeatText.isNotEmpty) {
        ...
        if (returnToHeatDate != null &&
          ...
          if (updated) {
            debugPrint('Cow ${_cattleDetails!.tagNo} status auto-updated to Healthy');
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
      ...
      debugPrint('DEBUG: Current cattle details: ${_cattleDetails?.tagNo}');
      ...
      debugPrint('DEBUG: Bull tag controller value: ${_controllers['bull_tag']?.text}');
  ...
  Future<void> _createReciprocalBreedingEvent() async {
    ...
    try {
      ...
      debugPrint('  - bull_tag: ${_controllers['bull_tag']?.text}');
      ...
      if (breedingType == 'artificial_insemination') {
        ...
        if (semenUsed.isNotEmpty) {
          ...
          if (semenUsed.contains(' Semen')) {
            ...
            debugPrint('DEBUG: Extracted bull tag from semen label: $partnerCattleTag');
          } else {
            partnerCattleTag = semenUsed.trim();
            debugPrint('DEBUG: Using semen_used as pure tag for partner: $partnerCattleTag');
          }
        }
      } else if (breedingType == 'natural_breeding') {
        partnerCattleTag = _controllers['bull_tag']?.text ?? '';
        debugPrint('DEBUG: Bull tag from natural breeding: $partnerCattleTag');
      } else {
      ...
      if (partnerCattle == null) {
        debugPrint('Partner cattle not found: $partnerCattleTag');
        return;
      }
      ...
      if (partnerSex != 'male') {
        debugPrint('Partner cattle is not male, skipping reciprocal breeding event');
        return;
      }
      ...
      if (eventDate.isNotEmpty) {
        ...
        debugPrint('DEBUG: Checking for existing history for partner: $partnerCattleTag');
        debugPrint('DEBUG: Found ${existingEvents.length} history for partner');
        
        // Filter history to only include those for the specific partner cattle
        final partnerEvents = existingEvents.where((event) => 
          event['cattle_tag']?.toString() == partnerCattleTag
        ).toList();
        
        debugPrint('DEBUG: Filtered to ${partnerEvents.length} history for partner cattle');
        ...
        if (hasExistingBreedingEvent) {
          debugPrint('DEBUG: Breeding event already exists for partner cattle on this date');
          return;
        } else {
      ...
      final result = await CattleHistoryService.storeCattleHistory(reciprocalEventData);
      
      if (result) {
        debugPrint('Successfully created reciprocal breeding event for $partnerCattleTag');
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Reciprocal breeding history record created for $partnerCattleTag'),
        ...
        debugPrint('Failed to create reciprocal breeding event for $partnerCattleTag');
      }
  ...
  Future<void> _handlePregnantEvent() async {
    try {
      ...
      // Log the result for debugging
      debugPrint('Pregnant event result: $cowStatus');
    ...
    try {
      ...
      if (_cattleDetails != null) {
        ...
        if (cattleStatusUpdated) {
          ...
          debugPrint('Auto-archive result for mortality cattle: $cattleArchived');
        }
      }

      // Log the result for debugging
      debugPrint('Mortality event result: $cattleStatus');
      ...
      if (mounted && cattleStatusUpdated) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(message),
            backgroundColor: cattleArchived ? Colors.green[600] : Colors.grey[700],
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
            duration: const Duration(seconds: 3),
          ),
  ...
  Future<void> _handleWeighedEvent() async {
    try {
      ...
      if (latestWeight == null || latestWeight <= 0) {
        debugPrint('Weighed event: invalid or empty weight, skipping cattle weight update');
        return;
      }
      ...
      debugPrint('Cattle weight update result: $updated');
    ...
    try {
      ...
      if (_cattleDetails != null) {
        ...
        if (cattleStatusUpdated) {
          ...
          debugPrint('Auto-archive result for lost cattle: $cattleArchived');
        }
      }

      // Log the result for debugging
      debugPrint('Lost event result: $cattleStatus');

      // Show success feedback to user
      if (mounted && cattleStatusUpdated) {
        final message = cattleArchived 
            ? 'Cattle status updated to Lost and moved to archive'
            : 'Cattle status updated to Lost';
        
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(message),
            backgroundColor: cattleArchived ? Colors.green[600] : Colors.amber[600],
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
            duration: const Duration(seconds: 3),
          ),
  ...
  Future<void> _handleSickEvent() async {
    try {
      ...
      // Log the result for debugging
      debugPrint('Sick event result: $cattleStatus');
  ...
  Future<void> _deleteEvent() async {
    ...
    if (confirmed == true) {
      setState(() => _isLoading = true);
      final success =
      await CattleHistoryService.deleteCattleHistory(widget.historyRecord!.id);
      if (!mounted) return;
      setState(() => _isLoading = false);
      if (success) {
        Navigator.pop(context, true);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          isEditing ? 'Edit Cattle History' : 'Add Cattle History',
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            HistorySpecificFields(
                              ...
                              controllers: _controllers,
                              cattleTag: widget.cattleTag ?? (isEditing && widget.historyRecord != null ? widget.historyRecord!.cattleTag : null),
