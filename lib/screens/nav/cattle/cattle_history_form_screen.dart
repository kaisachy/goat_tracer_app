...
class _CattleHistoryFormScreenState extends State<CattleHistoryFormScreen>
  ...
  void initState() {
    ...
    print('DEBUG: Checking if should load breeding history data');
    print('DEBUG: isEditing: $isEditing');
    print('DEBUG: widget.historyRecord: ${widget.historyRecord}');
    if (widget.historyRecord != null) {
      print('DEBUG: widget.historyRecord!.historyType: "${widget.historyRecord!.historyType}"');
    }
    
    if (isEditing && widget.historyRecord != null && widget.historyRecord!.historyType == 'Breeding') {
      print('DEBUG: Loading breeding history data...');
      // Use post-frame callback to ensure controllers are initialized
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _loadBreedingHistoryData();
      });
    } else {
      print('DEBUG: Not loading breeding history data - conditions not met');
    }
  }
  ...
  Future<void> _handleHistoryTypeChanged(String value) async {
    print('DEBUG: _handleHistoryTypeChanged called with value: $value');
    ...
    print('DEBUG: Processing history type change for cattle: $cattleTag');
    ...
    if (value.toLowerCase() == 'pregnant') {
      print('DEBUG: Processing Pregnant history record for cattle: $cattleTag');
      final latestBreeding = await _getLatestHistoryRecord(cattleTag: cattleTag, historyType: 'Breeding');
      print('DEBUG: Latest breeding history record found: $latestBreeding');
      if (latestBreeding == null) {
        print('DEBUG: No breeding history record found, showing warning');
    ...
    if (value.toLowerCase() == 'gives birth') {
      print('DEBUG: Processing Gives Birth history record for cattle: $cattleTag');
      final latestPregnant = await _getLatestHistoryRecord(cattleTag: cattleTag, historyType: 'Pregnant');
      print('DEBUG: Latest pregnant history record found: $latestPregnant');
      if (latestPregnant == null) {
        print('DEBUG: No pregnant history record found, showing warning');
      ...
      print('DEBUG: Found in pregnant history record - bull_tag: $bull, semen_used: $semen');
      if ((semen == null || semen.isEmpty) && (bull == null || bull.isEmpty)) {
        print('DEBUG: No sire info in pregnant history record, falling back to latest breeding');
        final latestBreeding = await _getLatestHistoryRecord(cattleTag: cattleTag, historyType: 'Breeding');
        print('DEBUG: Latest breeding history record for fallback: $latestBreeding');
  ...
  Future<void> _loadBreedingHistoryData() async {
    try {
      if (widget.historyRecord == null) {
        print('DEBUG: No history record to load breeding data from');
        return;
      }
      
      final eventJson = widget.historyRecord!.toJson();
      print('DEBUG: Loading breeding history data: $eventJson');
      print('DEBUG: History type: ${widget.historyRecord!.historyType}');
      print('DEBUG: Available fields in history record: ${eventJson.keys.toList()}');
      
      // Set breeding type (map human-readable <-> snake_case)
      final breedingType = eventJson['breeding_type']?.toString();
      print('DEBUG: Found breeding_type in history record: $breedingType');
      if (breedingType != null && breedingType.isNotEmpty) {
        ...
        print('DEBUG: No breeding_type found in history record data');
      ...
      print('DEBUG: Found semen_used in history record: $semenUsed');
      if (semenUsed != null && semenUsed.isNotEmpty) {
        ...
        print('DEBUG: No semen_used found in history record data');
      ...
      print('DEBUG: Found bull_tag in history record: $bullTag');
      if (bullTag != null && bullTag.isNotEmpty) {
        _controllers['bull_tag']?.text = bullTag;
        print('DEBUG: Set bull_tag controller to: $bullTag');
      } else {
        print('DEBUG: No bull_tag found in history record data');
      }
      
      // Set technician
      final technician = eventJson['technician']?.toString();
      print('DEBUG: Found technician in history record: $technician');
      if (technician != null && technician.isNotEmpty) {
        ...
        print('DEBUG: No technician found in history record data');
      }
      ...
      // Values read below directly from controllers when needed; avoid unused locals
        ...
        if (isMaleSubject && isBreeding && eventDate.isNotEmpty) {
          ...
          if (partnerTag != null && partnerTag.isNotEmpty) {
            print('DEBUG: Hydration - attempting to fetch partner history record for tag: $partnerTag on $eventDate');
            ...
            if (partner.isNotEmpty) {
              ...
              print('DEBUG: Hydration - no matching partner breeding history record found');
      ...
      print('Error loading breeding history data: $e');
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      print('Selected event type: $selectedHistoryType');
      ...
      try {
        if (isEditing) {
          ...
          print('Updating event with ID: ${widget.historyRecord!.id}');
          eventSuccess = await CattleHistoryService.updateCattleHistory(data);
        } else {
          print('Creating new history record');
      ...
      if (eventSuccess) {
        ...
        try {
          print('🎯 Event type handling - selectedHistoryType: "$selectedHistoryType"');
          if (selectedHistoryType.toLowerCase() == 'gives birth') {
            ...
            print('🎯 No specific event handler for: "$selectedHistoryType"');
          }
    ...
    try {
      ...
      if (_cattleDetails != null) {
        ...
        if (cattleStatusUpdated) {
          ...
          print('Auto-archive result for sold cattle: $cattleArchived');
        }
      }

      // Log the result for debugging
      print('Sold event result: $cattleStatus');
      ...
      // Log any errors that occur during the process
      print('Error in _handleSoldEvent: $e');
  ...
  Future<void> _createReciprocalBreedingEvent() async {
    ...
    try {
      ...
      if (eventDate.isNotEmpty) {
        ...
        print('DEBUG: Found ${existingEvents.length} history for partner');
    ...
    try {
      ...
      if (_cattleDetails != null) {
        ...
        if (cattleStatusUpdated) {
          ...
          print('Auto-archive result for lost cattle: $cattleArchived');
        }
      }
